...
智慧医药系统（smart-medicine）是一个基于 SpringBoot 开发的智能医药系统，为本科毕业设计项目。系统采用 SSM 架构模式，整合了通义千问大语言模型作为智能医生，提供医药信息查询与智能咨询服务。
...
- **阿里云 OSS 对象存储**
  ...
  阿里云对象存储 OSS（Object Storage Service）是一款海量、安全、低成本、高可靠的云存储服务，对于本系统而言，所有的二进制文件，包括头像、用户上传的文件都是存储到了 OSS 里面（调用相关的工具类），在数据库中只是保存了文件的 URL 地址。通过这个 URL 地址就可以获取、下载指定文件
...
> 注意！如果想要部署本系统，你只需要修改一个文件：application.yml
>
> 1. 修改数据源配置
> 2. 修改邮件配置
> 3. 阿里云通义千问配置
